...
public class AddWordActivity extends Activity {
	...
	private DialogInterface.OnClickListener dialogPositiveButtonClickListener = new DialogInterface.OnClickListener() {
		...
		public void onClick(DialogInterface dialog, int whichButton) {
			...
			EditText etJapaneseword = (EditText) findViewById(R.id.et_japaneseword);
			EditText etExtras = (EditText) findViewById(R.id.et_extras);
			...
			// 入力されている場合、データベースに登録
			// WordBeanインスタンス生成
			WordBean wbn = new WordBean(etEnglishword.getText().toString(), etJapaneseword.getText().toString(), etExtras.getText().toString());
	...
	private void showDeleteDialog() {
		...
		AlertDialog.Builder dialog = new AlertDialog.Builder(AddWordActivity.this);
		// ダイアログタイトル設定
		dialog.setTitle(R.string.delete_confirm_text);
		// ダイアログメッセージ設定
		dialog.setMessage(R.string.deleteconfirm_message_text);
		// 「はい」ボタン設定
		dialog.setPositiveButton(R.string.yes_text, deleteDialogPositive);
		// 「いいえ」ボタン設定
		dialog.setNegativeButton(R.string.no_text, deleteDialogNegative);
		// ダイアログ表示
		dialog.show();
	}
	
	private DialogInterface.OnClickListener deleteDialogPositive = new DialogInterface.OnClickListener() {
		
		@Override
		public void onClick(DialogInterface dialog, int which) {
			dbUtil.deleteWord();
			Toast.makeText(AddWordActivity.this, "削除完了しました！", Toast.LENGTH_SHORT).show();
		}
	};
	
	private DialogInterface.OnClickListener deleteDialogNegative = new DialogInterface.OnClickListener() {
